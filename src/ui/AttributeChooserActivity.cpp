...
AttributeChooserActivity::AttributeChooserActivity(std::vector<AttributeChoice> &items, String title, uint8_t selected, bool showTitleBar, int titleFontScale, bool showArrowHome)
:Activity(title, showTitleBar, titleFontScale, showArrowHome), items(items), selectedItem(selected) {
...
void AttributeChooserActivity::drawLayout() {
    ...
    display.fillRect(36, titleBarHeight, display.width() - 36, display.height() - titleBarHeight, BLACK);
    for(int8_t i = 0; i < 4 && itemsIT != items.end(); i++) { 
        if(i != 0 || i == 0 && selectedItem != 0) { // skip the first position, when the first item is selected
