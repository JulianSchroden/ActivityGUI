...
protected:
	...
	/**
	 *  Helper method to draw the selection indicator. Sets the lastSelectionIndicator to the passed index
...
public:
	 ...
	 *  @param items            a reference to a std::list containing the strings, which should be displayed as list items
	 ...
	 */ 
	ListActivity(std::list<String> &items, String title, bool showTitleBar, int titleFontScale, bool showArrowHome = false);

	virtual void onScroll(int distance);
	virtual void onClick();

	/**
	 *  virtual callback method, which is called by onClick and should be implemented in the derived class
