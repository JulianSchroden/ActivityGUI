...
Activity::Activity(String title, bool showTitleBar, int titleFontScale, bool showArrowHome) 
:display(Runtime::getDisplay()), title(title), showTitleBar(showTitleBar), titleFontScale(titleFontScale), showArrowHome(showArrowHome) {
...
void Activity::drawTitleBar(String title, int fontScale, bool showArrowHome, bool draw) {
	...
	if (showArrowHome) {
		display.fillTriangle(4, titleBarHeight / 2, 12, (titleBarHeight / 2) - 4, 12, (titleBarHeight / 2) + 4, BLACK);
	}
...
void Activity::clearActivity(const int color, const boolean draw) {
	...
	display.fillRect(0, titleBarHeight, display.width(), display.height() - titleBarHeight, color);
	if (draw) {
		display.display();
	}
}
