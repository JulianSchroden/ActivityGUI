...
ListActivity::ListActivity(std::list<String> &items, String title, bool showTitleBar, int titleFontScale, bool showArrowHome)
:Activity(title, showTitleBar, titleFontScale, showArrowHome), listItems(items){
...
void ListActivity::drawLayout() {
	...
	for (int i = 0; i <= visibleItemCount; i++) {
		...
		display.fillRect(listItemHeight + 4, titleBarHeight + i * listItemHeight + 3, display.width() - listItemHeight - 4 - 8, listItemHeight - 5, BLACK);
		
		if (newIT != listItems.end()) { 
			//draw indicator background
			display.drawRect(4, titleBarHeight + i * listItemHeight + 2, listItemHeight - 4, listItemHeight - 4, WHITE);
			//draw text
			display.setCursor(listItemHeight + 4, titleBarHeight + i * listItemHeight + 3);
			display.println(*newIT);
			//Draw list seperator
			display.drawFastHLine(0, titleBarHeight + (i + 1) * listItemHeight, display.width() - scrollBarWidth, WHITE);
			newIT++;
		}
	...
	if (selectedItem == maxIndex) {
		display.drawRect(4, titleBarHeight + visibleItemCount*listItemHeight + 2, listItemHeight - 4, listItemHeight - 4, BLACK);
	}
	//redrawing the last indicator background, when the item before is selected
	else if (selectedItem == maxIndex - 1) {
		display.drawRect(4, titleBarHeight + visibleItemCount*listItemHeight + 2, listItemHeight - 4, listItemHeight - 4, WHITE);
	}
...
void ListActivity::drawScrollIndicator(const boolean draw) {
	//draw background
	display.fillRect(display.width() - scrollBarWidth, titleBarHeight, scrollBarWidth, display.height() - titleBarHeight, WHITE);

	//draw arrows
	display.fillTriangle(display.width() - scrollBarWidth + 1, titleBarHeight + 3, display.width() - ceil((float)scrollBarWidth / 2.0f), titleBarHeight, display.width() - 2, titleBarHeight + 3, BLACK);
	display.fillTriangle(display.width() - scrollBarWidth + 1, display.height() - 4, display.width() - ceil((float)scrollBarWidth / 2.0f), display.height() - 2, display.width() - 2, display.height() - 4, BLACK);

	//draw Indicator
	int x = display.width() - ceil((float)scrollBarWidth / 2.0f) - (indicatorWidth / 2);
	int y = titleBarHeight + 5 + ((float)selectedItem / (float)maxIndex) * (display.height() - titleBarHeight - 11) - ((float)selectedItem / (float)maxIndex) * 5.0f;
...
void ListActivity::drawSelectionIndicator(const int index, const boolean draw) {
	...
	display.fillRect(6, titleBarHeight + lastSelectionIndicator * listItemHeight + 4, listItemHeight - 8, listItemHeight - 8, BLACK);
	// draw indicator of the current selected item
	display.fillRect(6, titleBarHeight + index * listItemHeight + 4, listItemHeight - 8, listItemHeight - 8, WHITE);
	if (draw) {
		display.display();
	}
...
void ListActivity::onScroll(int distance) {
	...
	if (selectedItem != lastSelectedItem) {
		...
		if ((selectedItem > selectionIndicator) || (selectedItem < visibleItemCount && lastSelectedItem >= visibleItemCount)) {
			drawLayout();
		}
